...
UpdateVelocity(Ptr < Node > node0) {
    ...
    mobility - > SetVelocity(Vector(uv - > GetValue(30, 50), 50.0, 0.0));
...
NotifyConnectionEstablishedUe(std::string context,
    uint64/* -*-  Mode: C++; c-file-style: "gnu"; indent-tabs-mode:nil; -*- */
/*
 * Copyright (c) 2013 Centre Tecnologic de Telecomunicacions de Catalunya (CTTC)
 *
 * This program is free software; you can redistribute it and/or modify
...
NotifyHandoverStartUe(std::string context,
    ...
    std::cout << Simulator::Now().GetSeconds() << context << " UE IMSI " << imsi << ": previously connected to CellId " << cellid << " with RNTI " << rnti << ", doing handover to CellId " << targetCellId << std::endl;
}
...
NotifyHandoverEndOkUe(std::string context,
    ...
    uint16_t cellid,
    uint16_t rnti) {
    std::cout << Simulator::Now().GetSeconds() << context << " UE IMSI " << imsi << ": successful handover to CellId " << cellid << " with RNTI " << rnti << std::endl;
}
...
NotifyHandoverStartEnb(std::string context,
    ...
    uint16_t rnti,
    uint16_t targetCellId) {
    std::cout << Simulator::Now().GetSeconds() << context << " eNB CellId " << cellid << ": start handover of UE with IMSI " << imsi << " RNTI " << rnti << " to CellId " << targetCellId << std::endl;
}
...
NotifyHandoverEndOkEnb(std::string context,
    ...
    uint16_t cellid,
    uint16_t rnti) {
    std::cout << Simulator::Now().GetSeconds() << context << " eNB CellId " << cellid << ": completed handover of UE with IMSI " << imsi << " RNTI " << rnti << std::endl;
}
...
main(int argc, char * argv[]) {
    ...
    // Routing of the Internet Host (towards the LTE network)
    Ipv4StaticRoutingHelper ipv4RoutingHelper;
    Ptr < Ip/* -*-  Mode: C++; c-file-style: "gnu"; indent-tabs-mode:nil; -*- */
/*
 * Copyright (c) 2013 Centre Tecnologic de Telecomunicacions de Catalunya (CTTC)
 *
 * This program is free software; you can redistribute it and/or modify
...
no matchesv4StaticRouting > remoteHostStaticRouting = ipv4RoutingHelper.GetStaticRouting(remoteHost - > GetObject < Ipv4 > ());
    // interface 0 is localhost, 1 is the p2p device
    remoteHostStaticRouting - > AddNetworkRouteTo(Ipv4Address("7.0.0.0"), Ipv4Mask("255.0.0.0"), 1);
